...
serve(async (req) => {
  ...
  try {
    ...
    const rotationDegrees = await detectImageOrientation(preparedImage.base64Image, anthropicApiKey, preparedImage.mediaType, anthropicModel)
    ...
    if (rotationDegrees > 0) {
      ...
      try {
        ...
        if (decodedImage) {
          // imagescript rotate()는 반시계방향 기준이므로, Claude가 반환한 시계방향 각도를 변환
...
async function detectImageOrientation(base64Image: string, apiKey: string, mediaType: "image/png" | "image/jpeg" = "image/jpeg", model = "claude-opus-4-5"): Promise<number> {
  try {
...
18) 테이블의 모든 행을 있는 그대로 추출할 것. 품명 칸에 "ENIG(화학금도금)", "필름", "V-CUT", "네고", "잉크비" 등 부자재/공정/할인 항목이 있으면 그대로 별도 행으로 포함. 품명 칸이 완전히 비어있어도 금액이 있으면 별도 행으로 포함 (직전 품명으로 병합 금지)
19) line_number는 거래명세서의 No/번호 칼럼에 인쇄된 숫자를 그대로 사용할 것 (예: 29, 30, 43 등). 1부터 오름차순으로 임의 부여 금지. 번호 칼럼이 없으면 행 순서대로 1부터 부여
