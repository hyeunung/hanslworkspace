...
import {
  BOM_BOARD_FILTER_FIELDS, BomBoardFilterRule, BomBoardFilterOp, BomBoardOptionsKey,
  bomBoardFieldDefFor, bomBoardFieldLabel, bomBoardOpLabel, opsForBomBoardField,
  bomBoardSelectOptions, defaultBomBoardRuleForField,
} from '@/utils/bomBoardTable'
import { BomBoardFilterConfig } from '@/hooks/useBomBoardFilterViews'
import { measureText } from '@/utils/productionColumns'
import { AnchoredPortal } from '@/components/ui/AnchoredPortal'

interface BomBoardFilterToolbarProps {
  rules: BomBoardFilterRule[]
  dynamicOptions: Partial<Record<BomBoardOptionsKey, string[]>>
  years: number[]
  addRule: () => void
  updateRule: (id: string, patch: Partial<BomBoardFilterRule>) => void
  changeRuleField: (id: string, field: string) => void
  removeRule: (id: string) => void
  resetRules: () => void
  // 저장뷰 (useBomBoardTableFilters에서 주입)
  filterViewsConfig: BomBoardFilterConfig
...
// 보드별 정리 필터 툴바 (노션식 규칙 필터) — PurchaseFilterToolbar를 보드별 정리 필드로 주입한 버전.
// 규칙 = [칼럼 ▾][조건 ▾][값 입력][×], 저장된 필터/기본값 메뉴 포함. 디자인·동작 동일.
export default function BomBoardFilterToolbar({
  ...
  const changeRuleOp = (rule: BomBoardFilterRule, op: BomBoardFilterOp) => {
    if (op === 'month_in') {
    ...
    const keepValue = def?.type === 'select' ? (rule.value ?? defaultBomBoardRuleForField(rule.field, dynamicOptions).value) : ''
          ...
          {rules.map(rule => {
            ...
            return (
              <div key={rule.id} className="hansl-filter-pill">
                ...
                <select
                  value={rule.op}
                  onChange={(e) => changeRuleOp(rule, e.target.value as BomBoardFilterOp)}
                  style={fitSelect(bomBoardOpLabel(rule.op))}
                  className={selectClass}
                >
                  {ops.map(op => (
                    <option key={op} value={op}>{bomBoardOpLabel(op)}</option>
                  ))}
                </select>
