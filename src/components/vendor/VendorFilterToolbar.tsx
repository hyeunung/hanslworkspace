...
import {
  VENDOR_FILTER_FIELDS, VendorFilterRule, VendorFilterOp, VendorOptionsKey,
  vendorFieldDefFor, vendorFieldLabel, vendorOpLabel, opsForVendorField,
  vendorSelectOptions, defaultVendorRuleForField,
} from '@/utils/vendorTable'
import { VendorFilterConfig } from '@/hooks/useVendorFilterViews'
...
interface VendorFilterToolbarProps {
  ...
  years: number[]
  addRule: () => void
  updateRule: (id: string, patch: Partial<VendorFilterRule>) => void
  changeRuleField: (id: string, field: string) => void
  removeRule: (id: string) => void
  resetRules: () => void
  // 저장뷰 (useVendorTableFilters에서 주입)
  filterViewsConfig: VendorFilterConfig
...
// 업체관리 필터 툴바 (노션식 규칙 필터) — BomBoardFilterToolbar를 업체 필드로 주입한 버전.
// 규칙 = [칼럼 ▾][조건 ▾][값 입력][×], 저장된 필터/기본값 메뉴 포함. 디자인·동작 동일.
export default function VendorFilterToolbar({
  ...
  const changeRuleOp = (rule: VendorFilterRule, op: VendorFilterOp) => {
    if (op === 'month_in') {
    ...
    const keepValue = def?.type === 'select' ? (rule.value ?? defaultVendorRuleForField(rule.field, dynamicOptions).value) : ''
          ...
          {rules.map(rule => {
            ...
            return (
              <div key={rule.id} className="hansl-filter-pill">
                ...
                <select
                  value={rule.op}
                  onChange={(e) => changeRuleOp(rule, e.target.value as VendorFilterOp)}
                  style={fitSelect(vendorOpLabel(rule.op))}
                  className={selectClass}
                >
                  {ops.map(op => (
                    <option key={op} value={op}>{vendorOpLabel(op)}</option>
                  ))}
                </select>
