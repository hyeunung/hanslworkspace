...
import { VENDOR_COLUMN_LABELS, VendorColumnId } from '@/utils/vendorTable'
import { AnchoredPortal } from '@/components/ui/AnchoredPortal'

export type VendorColumnVisibility = Partial<Record<VendorColumnId, boolean>>

interface VendorColumnMenuProps {
  columnVisibility: VendorColumnVisibility
  toggleColumn: (columnId: VendorColumnId) => void
  resetToDefault: () => void
}

// 칼럼 표시 설정 드롭다운 — BomBoardColumnMenu 형식(클릭 즉시 반영 + 자동 저장 localStorage).
// 업체관리는 칼럼이 적어 그룹 없이 단일 목록. 업체명은 필수(잠금).
export default function VendorColumnMenu({
  columnVisibility, toggleColumn, resetToDefault,
}: VendorColumnMenuProps) {
  const [open, setOpen] = useState(false)
  const [anchorEl, setAnchorEl] = useState<HTMLElement | null>(null)

  const total = VENDOR_COLUMN_LABELS.length
  const shownCount = VENDOR_COLUMN_LABELS.filter(c => columnVisibility[c.id] !== false).length
          ...
          <AnchoredPortal anchorEl={anchorEl} align="right" gap={4}>
            <div className="hansl-popover pb-2 w-[280px]">
              ...
              <div className="px-3 pt-2 grid grid-cols-2 gap-x-2">
                {VENDOR_COLUMN_LABELS.map(({ id, label, required }) => {
