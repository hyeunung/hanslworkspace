...
export default function BusinessTripTab({ mode = "list", onBadgeRefresh }: BusinessTripTabProps) {
  ...
  const createEmptyExpense = useCallback((trip?: BusinessTrip | null, dateStr?: string): ExpenseFormRow => ({
    key: newKey(),
  ...
  const createEmptyAllowance = useCallback((): AllowanceFormRow => ({
    key: newKey(),
    region: "Domestic",
    day_count: "",
    unit_amount: "",
    ...
    try {
      ...
      if (expensePayload.length > 0) {
        ...
        for (let i = 0; i < expenseRowsForSave.length; i += 1) {
          ...
          if (row.expense_type === "corporate_card" && linkedCardUsageId && rowReceiptPathsForCardUsage.length > 0) {
  ...
  return (
    <div className="w-full">
                          ...
                          {trip.requested_card_number?.trim() || trip.linkedCard ? (
                            <div>
                              <div className="card-title">
                                {trip.requested_card_number?.split(" ")[0] || trip.linkedCard?.card_number?.split(" ")[0] || "-"}
                              </div>
      ...
      {/* 정산 모달 */}
      <Dialog open={!!settlementTrip} onOpenChange={(open) => !open && closeSettlementModal()}>
      ...
      <div className="border business-radius-card">
                <div className="px-3 py-2 border-b bg-gray-50 flex items-center justify-between">
                  ...
                  {!settlementViewOnly && (
                    <Button
                      onClick={() => setExpenseRows((prev) => [...prev, createEmptyExpense(settlementTrip)])}
                      className="button-base border border-gray-300 bg-white text-gray-700 hover:bg-gray-50"
                    >
                      + 행 추가
                    </Button>
                                  ...
                                  <span
                                    ...
                                    onKeyDown={(e) => {
                                      if (e.key === 'Enter' || e.key === ' ') {
                                        if (settlementViewOnly) openDirectReceipt(row.existingReceipts);
              ...
              {/* 일비 */}
              <div className="border business-radius-card" style={{ width: "max-content", minWidth: "440px" }}>
                <div className="px-3 py-2 border-b bg-gray-50 flex items-center justify-between">
