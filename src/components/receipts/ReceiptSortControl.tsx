...
import { ReceiptSortRule, RECEIPT_SORT_FIELDS, receiptSortLabel } from '@/utils/receiptTable'
import { measureText } from '@/utils/productionColumns'
import { AnchoredPortal } from '@/components/ui/AnchoredPortal'

interface ReceiptSortControlProps {
  sortRules: ReceiptSortRule[]
  addSortRule: () => void
  updateSortRule: (id: string, patch: Partial<ReceiptSortRule>) => void
  removeSortRule: (id: string) => void
  clearSort: () => void
}

// 정렬 컨트롤 (노션식) — BomBoardSortControl을 영수증 칼럼으로 주입한 버전. JSX/동작 동일.
export default function ReceiptSortControl({
  sortRules, addSortRule, updateSortRule, removeSortRule, clearSort,
}: ReceiptSortControlProps) {
            ...
            <div className="hansl-popover w-max min-w-[200px] max-w-[340px]">
                  ...
                  <div key={r.id} className="flex items-center gap-1.5">
                    ...
                    <div className="relative shrink-0">
                      <select
                        ...
                        style={{ padding: '0 15px 0 7px', lineHeight: '20px', backgroundImage: 'none', width: `${Math.ceil(measureText(receiptSortLabel(r.field), 400) * 1.1) + 24}px` }}
                        className="hansl-select-box"
                      >
                        {RECEIPT_SORT_FIELDS.map(f => (
              ...
              <div className="px-2 pb-2 pt-1 border-t border-gray-100">
                <button
                  type="button"
                  onClick={addSortRule}
                  disabled={sortRules.length >= RECEIPT_SORT_FIELDS.length}
