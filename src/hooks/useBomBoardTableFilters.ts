...
import {
  BomBoardFilterRule, StoredBomBoardFilterRule, BomBoardOptionsKey,
  newBomBoardRuleId, defaultBomBoardRuleForField,
} from '@/utils/bomBoardTable'
import { useBomBoardFilterViews, SavedBomBoardFilterView } from '@/hooks/useBomBoardFilterViews'

// 보드별 정리 노션식 필터 상태 훅 — 규칙 CRUD + 저장뷰(DB) 오케스트레이션.
// usePurchaseTableFilters와 동일 골격 (규칙은 세션 전용, 저장 필터/기본값만 DB 동기화).

const withNewIds = (rules: StoredBomBoardFilterRule[]): BomBoardFilterRule[] =>
  rules.map(r => ({ ...r, id: newBomBoardRuleId() }))

export function useBomBoardTableFilters(
  dynamicOptions: Partial<Record<BomBoardOptionsKey, string[]>>
) {
  const [rules, setRules] = useState<BomBoardFilterRule[]>([])
  const filterViews = useBomBoardFilterViews()
  ...
  const addRule = useCallback(() => {
    setRules(prev => [...prev, { id: newBomBoardRuleId(), field: '', op: 'contains', value: '' }])
  }, [])

  const updateRule = useCallback((id: string, patch: Partial<BomBoardFilterRule>) => {
    setRules(prev => prev.map(r => (r.id === id ? { ...r, ...patch } : r)))
  }, [])

  const changeRuleField = useCallback((id: string, field: string) => {
    setRules(prev => prev.map(r =>
      r.id === id ? { id: r.id, ...defaultBomBoardRuleForField(field, dynamicOptions) } : r))
  }, [dynamicOptions])
  ...
  const snapshotRules = useCallback((): StoredBomBoardFilterRule[] =>
  ...
  const commitSaveView = useCallback(async () => {
    ...
    closeViewsMenu()
    try {
      const view: SavedBomBoardFilterView = {
