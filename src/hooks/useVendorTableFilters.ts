...
import {
  VendorFilterRule, StoredVendorFilterRule, VendorOptionsKey,
  newVendorRuleId, defaultVendorRuleForField,
} from '@/utils/vendorTable'
import { useVendorFilterViews, SavedVendorFilterView } from '@/hooks/useVendorFilterViews'

// 업체관리 노션식 필터 상태 훅 — 규칙 CRUD + 저장뷰(DB) 오케스트레이션.
// useBomBoardTableFilters와 동일 골격 (규칙은 세션 전용, 저장 필터/기본값만 DB 동기화).

const withNewIds = (rules: StoredVendorFilterRule[]): VendorFilterRule[] =>
  rules.map(r => ({ ...r, id: newVendorRuleId() }))

export function useVendorTableFilters(
  dynamicOptions: Partial<Record<VendorOptionsKey, string[]>>
) {
  const [rules, setRules] = useState<VendorFilterRule[]>([])
  const filterViews = useVendorFilterViews()
  ...
  const addRule = useCallback(() => {
    setRules(prev => [...prev, { id: newVendorRuleId(), field: '', op: 'contains', value: '' }])
  }, [])

  const updateRule = useCallback((id: string, patch: Partial<VendorFilterRule>) => {
    setRules(prev => prev.map(r => (r.id === id ? { ...r, ...patch } : r)))
  }, [])

  const changeRuleField = useCallback((id: string, field: string) => {
    setRules(prev => prev.map(r =>
      r.id === id ? { id: r.id, ...defaultVendorRuleForField(field, dynamicOptions) } : r))
  }, [dynamicOptions])
  ...
  const snapshotRules = useCallback((): StoredVendorFilterRule[] =>
  ...
  const commitSaveView = useCallback(async () => {
    ...
    closeViewsMenu()
    try {
      const view: SavedVendorFilterView = {
