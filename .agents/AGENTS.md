...
* 높이가 20px로 작아짐에 따라 글씨 폰트는 `11px`, 안쪽 상하 여백은 `padding-top/bottom: 1px !important`로 맞춰 글자 짤림을 방지합니다. **또한 좌우 안쪽 여백은 좌우 6px (`padding-left/right: 6px !important`)로 일괄 정의되어 글자 밀착 규격이 자동으로 적용됩니다.**

### 3) 가로폭 밀착 및 정렬 (Content-Fit Width & Align)
* "핏하게" 또는 "텍스트 길이에 맞게"라는 뉘앙스의 요청이 있을 경우, 인풋 박스 및 테이블 컬럼의 가로폭 결정 시 **텍스트 좌우로 정확히 `6px`씩 여백(총 12px)**을 비우는 여백 규칙을 철저히 준수합니다 (이는 `.compact-inputs` 및 `.compact-modal` 하위 입력 요소에 기본 설정되어 있습니다):
  * **아이콘이 없는 일반 인풋 / 선택 창**: `.compact-inputs` 또는 `.compact-modal` 클래스에 의해 좌우 6px 패딩이 자동으로 적용되므로 별도의 Tailwind 패딩 클래스(pl, pr)를 선언하지 않고, 동적 너비는 `글자 길이 너비 + 12px`로 계산하여 주입합니다.
  * **돋보기(Search) 아이콘이 내장된 검색창**: 좌측 패딩 `30px` (돋보기 영역 24px + 좌측 여백 6px = 30px)을 위해 인라인 스타일로 `paddingLeft: '30px'`만 선언하고(우측 6px은 자동 적용), 동적 너비는 `글자 길이 너비 + 36px`로 계산합니다.
...
* 텍스트 라벨이 있는 인풋 옆에 나란히 배치되는 추가/삭제 버튼 등은 부모 flex/grid 레이아웃 상에서 **`items-end`** 또는 **`self-end`** 정렬을 주어 버튼 and 인풋의 바닥 라인이 일치하도록 수평 수평선을 맞춰줍니다.
