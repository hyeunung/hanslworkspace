-- support_inquires 테이블 RLS 정책 (테이블명 수정 버전)
-- app_admin 권한을 가진 사용자만 수정/삭제 가능

-- 1. support_inquires 테이블 정책
-- 기존 정책 모두 삭제
DROP POLICY IF EXISTS "Anyone can view inquiries" ON public.support_inquires;
DROP POLICY IF EXISTS "Anyone can create inquiries" ON public.support_inquires;
DROP POLICY IF EXISTS "Anyone can update inquiries" ON public.support_inquires;
...
DROP POLICY IF EXISTS "Users can view own inquiries" ON public.support_inquires;
DROP POLICY IF EXISTS "Admins can update inquiries" ON public.support_inquires;

-- RLS 활성화
ALTER TABLE public.support_inquires ENABLE ROW LEVEL SECURITY;

-- 본인 문의는 누구나 조회 가능
CREATE POLICY "Users can view own inquiries" ON public.support_inquires
FOR SELECT USING (
  auth.uid() = user_id 
  OR 
  EXISTS (
...
CREATE POLICY "Users can create inquiries" ON public.support_inquires
FOR INSERT WITH CHECK (auth.role() = 'authenticated');

-- app_admin만 문의 수정 가능
CREATE POLICY "Admins can update inquiries" ON public.support_inquires
FOR UPDATE USING (
  EXISTS (
    SELECT 1 FROM public.employees 
    WHERE email = auth.jwt() ->> 'email'
...
DROP POLICY IF EXISTS "Users can view items" ON public.purchase_request_items;
DROP POLICY IF EXISTS "Admins can update items" ON public.purchase_request_items;
DROP POLICY IF EXISTS "Admins can delete items" ON public.purchase_request_items;
...
DROP POLICY IF EXISTS "Users can view requests" ON public.purchase_requests;
DROP POLICY IF EXISTS "Admins can delete requests" ON public.purchase_requests;
